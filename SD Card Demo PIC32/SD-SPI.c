...
/*
void SPI2Putc(BYTE data)
{    
    while (!SPI2STATbits.SPITBE); // Wait for 
    SPI2BUF = data; // Send byte to SPI2BUF.
}   

unsigned int SPI2Getc(void)
{
	while (!SPI2STATbits.SPIRBF); // Wait for 
	return (unsigned int) SPI2BUF;	// return data in the buffer
}
*/
...
#endif
{
    ...
    //SPI module initilization depends on processor type
            ...
            SPIBRG = 1;
            // printf("\r>SPIRG High Frequency: %d", (int)SPIBRG);  // $$$$
        #endif
        SPICON1bits.CKP = 1;
        SPICON1bits.CKE = 0;
...
BYTE SPI_SendReceive(BYTE data)
{
    SPI2BUF = data; // Send byte to SPI2BUF.
    
#ifdef USE_PIC32_PINGUINO_MICRO    
    while (!SPI2STATbits.SPIRBF); // Wait for receive buffer   $$$$
#else
    while (SPI2STATbits.SPIRBE); // Wait for     
#endif    
    return (BYTE)SPI2BUF; // Return byte in SPI2BUF
}   
 ...
 */
            ...
            // printf("\r>OPEN SPI 2 MANUAL INIT SPIBRG: %ld", SPIBRG);    
...
#ifdef SD_DIAGNOSTICS
    printf("\rMDD_SDSPI_MediaInitialize timeout: %d", timeout);
#endif    
